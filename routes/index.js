...
exports.index = function(req, res){
	...
	client.get("https://api.mongolab.com/api/1/databases/bhavin/collections/gumballs?apiKey=4_NROT-a9g6nrAEScnNvLN5M3KGlDQpE",function(doc, resp){
		//console.log(data);
...
exports.GumballAction=function(req,res){
	...
	if(event==='InsertQuater' && state==='NoCoinState'){
		...
		if(hashnew==hash)
			...
			{
				res.render('index', { modelNumber : "Info corrupted, Please Restart", serialNumber : "Please Restart", state:"NoCoinState", count : "Please Restart",hashmsg: hash,id:id});
			}
	...
	if(event==='TurnTheCrank' && state==='HasACoin'){
		...
		if(hashnew==hash && count>0)
			{
				...
				var myCollection;
					...
					client.put("https://api.mongolab.com/api/1/databases/bhavin/collections/gumballs?apiKey=4_NROT-a9g6nrAEScnNvLN5M3KGlDQpE", args, function(data, response){
		...
		else
			{
				if(count == 0)
					...
					{
						res.render('index', { modelNumber : "Please Restart", serialNumber : "Please Restart", state:"NoCoinState", count : "Please Restart",hashmsg: hash,id:id});
					}
			}
		...
		console.log("modelNumber:"+modelNumber);
			if(event==='TurnTheCrank' && state==='NoCoinState'){
				res.render('index', { modelNumber : "1", serialNumber : "123456789999", state:"NoCoinState", count : "Info corrupted, cannot display the coin value",hashmsg: hash,id:id});
			}
		}
